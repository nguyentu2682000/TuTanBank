...
public interface BankAccountRepository extends CrudRepository<BankAccountEntity, Long>{
    ...
    @Query(value = "SELECT * FROM bankaccount as bk "
            + "join customer as c "
            + "on c.id = bk.customerId "
            + "where ci_number = ?1 or account_number = ?2", nativeQuery =true)
    BankAccountEntity findBankAccountbyCiNumberByAccountNumber(String CiNumber, String AccountNumber);
    
    @Query("SELECT ba FROM BankAccountEntity ba WHERE ba.customer.id =?1")
    BankAccountEntity searchBankAccountByCustomerId(long customerId);        
